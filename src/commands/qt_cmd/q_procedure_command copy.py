...
class QLockCmd(QSerialCmd):
    ...
    def _create_gui(self, columns_nb) -> None:
        ...
        self._send_button = QPushButton(self.BUTTON_TXT_START)
        self._send_button.setStyleSheet("background-color: green")
    ...
    def _on_procedure_button_clicked(self):
        if self._send_button.text() == self.BUTTON_TXT_START:
            self._send_button.setText(self.BUTTON_TXT_STOP)
            self._send_button.setStyleSheet("background-color: red; color: #FFFFFF")
            self._procedures_state = ProcedureCmdState.RUNNING
        else:
            self._send_button.setText(self.BUTTON_TXT_START)
            self._send_button.setStyleSheet("background-color: green; color: #FFFFFF")
            self._procedures_state = ProcedureCmdState.IDLE
            self._send_lock_widget.setChecked(False)
